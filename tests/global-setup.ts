...
// haex-pass extension public key file (generated by Docker build)
const HAEX_PASS_PUBLIC_KEY_FILE = "/app/haex-pass-public.key";

/**
 * Get the haex-pass public key from the generated file.
 * The Docker build generates a new keypair and updates the extension manifest.
 * We must use the same key here for database registration to match.
 */
function getHaexPassPublicKey(): string {
  const key = fs.readFileSync(HAEX_PASS_PUBLIC_KEY_FILE, "utf-8").trim();
  console.log("[Setup] Using haex-pass public key:", key.substring(0, 16) + "...");
  return key;
}

/**
 * Get the haex-pass manifest with the correct public key.
 */
function getHaexPassManifest() {
  return {
    name: "haex-pass",
    version: "1.4.31",
    author: "haex",
    ...
    publicKey: getHaexPassPublicKey(),
